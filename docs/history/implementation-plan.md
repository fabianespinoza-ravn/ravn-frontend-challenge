...
>
> **Last historical update:** 2026-08-19
...
> **Phase closure:** Board and list views were merged through [PR #15](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/15); Issue #14 is closed.
> **Phase closure:** The CI runtime chore was merged through [PR #17](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/17); Issue #16 is closed.
> **Phase closure:** The mobile filters fix was merged through [PR #19](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/19); Issue #18 is closed.
> **Phase closure:** The review remediation was merged through PRs [#22](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/22), [#24](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/24), [#26](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/26), [#28](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/28), [#30](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/30), [#32](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/32), [#34](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/34) and [#36](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/36); Issues #21, #23, #25, #27, #29, #31, #33 and #35 are closed. Its decisions are recorded in the maintained guides and in `../decisions/`, not here.
> **Current work:** None. Every issue is closed.
...
- Issue [#14 Board and list views](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/issues/14) is closed, was merged through [PR #15](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/15), and is in `Done` in the GitHub Project.
- Issue [#16 CI runtime](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/issues/16) is closed, was merged through [PR #17](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/pull/17), and is in `Done` in the GitHub Project.
- Issue [#18 Mobile task filters](https://github.com/fabianespinoza-ravn/ravn-frontend-challenge/issues/18) is the active work, on `fix/mobile-filters-viewport`.
...
| Drag and drop failure              | Restore or refetch the previous board state and show an error notification.        | **Superseded by contract 5.13:** `updateTask` rejects `position`, so reordering is blocked by the API and this handling was never built. |
...
### 7. Settings, Search and Filter, Views and CI

> Decisions 7.1 to 7.5 record the Settings phase. From 7.6 the same series continued through
> search and filtering, the shared status marker, the view toggle, the list's assignee column,
> the CI runtime chore and the mobile filters fix. The numbers are preserved exactly as they
> were recorded, because later decisions cite them.
...
- **7.18:** Below the layout breakpoint the filters panel is a dialog and its fields are rows, rather than an anchored panel whose fields anchor too. Measured in Chrome at `388px`, the panel opened `180px` past the right edge of the screen and each field between `120px` and `208px` past it, and the page gained the horizontal scrollbar the Dashboard UI phase had validated away at every width. 7.11 anchors the panel with `left: 0` and reasons that the trigger is not the last control in the toolbar, because the add-task button is; that is true at `768px` and above, and below it the toolbar hides that button and gives the view toggle the full width, so the trigger is the last control and sits flush against the edge. The reasoning was sound and its precondition was never written down, which is the correction this decision makes to 7.11 rather than a reversal of it. The fields compounded it, being `compact` and so anchored the same way. The presentation therefore changes rather than the anchor: the shared `Modal` carries the panel, and the fields take the `row` variant, whose options open centred over the screen behind a backdrop. 5.34 built that variant for the task form and recorded the identical reason — an anchored panel opened from a lower field would run off a phone viewport — so nothing here is new except that the filters panel, written later, never inherited it. Flipping the anchor to `right: 0` was rejected: it fits today only because the panels measure less than the screen, and every one of those widths is in `rem`, so a reader who enlarges their text scales the panel without scaling their phone, and no anchor rescues a panel wider than the screen; the dialog and the row overlay both cap themselves against the viewport instead. The `768px` breakpoint moves to a shared `useIsNarrowViewport`, which the layout now reads for the swap it already made at that width, so a second copy cannot drift from it — the same rule that sent `StatusDot` to `entities` in 7.14. jsdom computes no layout, so no assertion can measure the overflow, and none pretends to: the tests hold which presentation renders each side of the breakpoint and that the fields are rows below it, both confirmed to fail against the previous implementation. Verified in Chrome on both mobile presentations, which measured alike because the cause was shared, and at `768px`, where the anchored panel is unchanged.
- **7.17:** CI pins `actions/checkout@v7` and `actions/setup-node@v7`. Both were on `v4`, which declares `using: node20`; GitHub has retired that runtime and was forcing them onto Node 24 while annotating every run to say so. Nothing was failing, but that fallback is temporary and the annotation was on every check a reviewer would open. The current major was chosen over `v5`, the earliest that declares `node24`, because stopping at the oldest version that works only defers the same chore. Each intervening major was read rather than assumed: `setup-node@v5` began caching automatically when `package.json` names a `packageManager` and `v6` narrowed that to npm, neither of which reaches a project that declares no `packageManager` and asks for `cache: npm` outright, and `checkout@v7` restricts checking out fork pull requests under `pull_request_target` and `workflow_run`, which this workflow does not use. This is housekeeping rather than a requirement: it comes from GitHub's infrastructure and would appear in any repository pinning those versions.
- **7.16:** The list gains an assignee column, between the estimate and the due date, and only where rows can belong to different people. My Tasks leaves it out for the reason the static phase gave when it first omitted one: every row there is already the reader's own, so naming them would repeat the route. On the board's list it is not redundant, and it restores what the card view says through the avatar in its footer. An unassigned task says `Unassigned` rather than leaving the cell blank, since a blank reads as missing data instead of as an answer. The header row and the task rows take their tracks from one custom property the table swaps, so a column added to one cannot drift from the other; name and tags give up the width the new column needs and the table's minimum grows, which the existing horizontal scroll already handles.
...
- **7.7:** The search filters through the API rather than in the client. The server owns what a match is, and filtering here would only ever search whatever had already been loaded, which on My Tasks is one assignee's tasks and on the board is everything. My Tasks sends the name beside the assignee it already filtered by, which contract 5.10 confirmed combine with AND, so searching there stays inside what is assigned rather than escaping into everyone's tasks. An empty term sends no `name` at all rather than an empty one.
- **7.8:** The field holds one term for the whole shell, owned by the layout, because the control lives in the header and the queries live in the pages beneath it and neither can own it; this is 5.13's arrangement again. The term the field shows and the term the query uses are separate: the first follows every keystroke, the second waits 300ms for the typing to settle, so a typed word is one request rather than one per character. It is trimmed before it is applied, since a trailing space is a search nobody meant to run.
- **7.13:** The due date is the one filter applied after the response rather than in it. The API matches it as an exact instant, and the verified account holds two tasks due the same day stored at `02:02` and at noon, so a picker sending a chosen day could only ever find one of them. Filtering by calendar day in the client is exact rather than approximate, because the query has no pagination (5.5) and the client therefore holds the complete set its request asked for. Its test states the invariant instead of hardcoding it — the day the board shows a task under is the day that finds it — so it holds wherever the suite runs; written the other way it failed immediately in a timezone where `02:02Z` is the previous evening.
- **7.9:** A search that matches nothing says so, and is not confused with a board that has no tasks or an account with nothing assigned — they are different things to tell a reader, and only one of them suggests creating a task. That message also names the single surprise in how the search behaves: contract 5.10 recorded that the API matches the name **case-sensitively**, so a lowercase term will not find a capitalised task. The alternative, filtering case-insensitively in the client, was rejected because it would duplicate a rule the server owns and could only ever apply to the tasks already loaded. Telling the reader is the honest way to live with the API's semantics.
